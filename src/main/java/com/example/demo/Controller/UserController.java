...
//	}
	...
	public String dashboard(HttpSession session, Model model) {
	    ...
	    if (user != null) {
	        ...
	        String formattedLastLogin = user.getLastLogin() != null ? user.getLastLogin().format(formatter) : "Never";
	        model.addAttribute("lastLogin", formattedLastLogin);

	        // Handle profile image
	        if (user.getProfileImage() != null && user.getProfileImage().length > 0) {
	            String base64Image = Base64.getEncoder().encodeToString(user.getProfileImage());
	            String imageSrc = "data:" + MediaType.IMAGE_JPEG_VALUE + ";base64," + base64Image;
	            model.addAttribute("profileImage", imageSrc);
	        } else {
	            model.addAttribute("profileImage", "/images/default-profile.png"); // Default image if not available
	        }
